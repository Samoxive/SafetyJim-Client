...
export class GuildSettingsStore {
    isLoading = true;
    settings = new Map<string, GuildSettings>();
    ...
    async fetchSettings(loginStore: LoginStore, guild: Guild) {
        this.isLoading = true;
        await axios
            .get(`${apiUrl}/guilds/${guild.id}/settings`, {
    ...
    async updateSettings(
        ...
        await axios.post(`${apiUrl}/guilds/${guild.id}/settings`, newSettings, {
            headers: { token: loginStore.token }
        });
    ...
    async resetSettings(loginStore: LoginStore, guild: Guild) {
        await axios.delete(`${apiUrl}/guilds/${guild.id}/settings`, {
            headers: { token: loginStore.token }
        });
        await this.fetchSettings(loginStore, guild);
    }
