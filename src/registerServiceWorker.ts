...
export default function register() {
    if (process.env.NODE_ENV === 'production' && 'serviceWorker' in navigator) {
        ...
        const publicUrl =
            new URL(process.env.PUBLIC_URL!, window.location.toString());
        if (publicUrl.origin !== window.location.origin) {
            // Our service worker won't work if PUBLIC_URL is on a different
            // origin from what our page is served on. This might happen if a
            // CDN is used to serve assets; see
                    ...
                    installingWorker.onstatechange = () => {
                        if (installingWorker.state === 'installed') {
                            if (navigator.serviceWorker.controller) {
                                ...
                                // purged and the fresh content will have been
                                // added to the cache. It's the perfect time to
                                // display a 'New content is available; please
                                // refresh.' message in your web app.
                                console.log(
